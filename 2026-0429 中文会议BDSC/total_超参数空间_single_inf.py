...
def plot_upper_right_heatmap(df_deepseek, id_val, yd_val, save_path):
    """只绘制右上角的热力图 (ave_Sp vs ave_Sn)"""
    sns.set_style("white")
    plt.rcParams['axes.unicode_minus'] = False
    plt.rcParams['font.sans-serif'] = ['SimHei']  # 使用黑体


    x_col, y_col = "ave_Sp", "ave_Sn"
    ...
    ax.set_xlabel("平均正向固执度", fontsize=11)
    ax.set_ylabel("平均负向固执度", fontsize=11)

    plt.tight_layout()
    plt.savefig(save_path, dpi=600, bbox_inches='tight')
    plt.close()
    print(f"Saved upper right plot: {save_path}")

def plot_single_heatmap(df_deepseek, id_val, yd_val, save_path):
    """只绘制左上角的热力图 (ave_Ip vs ave_In)"""
    plt.rcParams['axes.unicode_minus'] = False
    plt.rcParams['font.family'] = 'Arial'
    sns.set_style("white")

    x_col, y_col = "ave_Ip", "ave_In"
    title = "Positive vs Negative Influence (Group Average)"
    ...
    plt.close()
    print(f"Saved single plot: {save_path}")
...
def main():
    # 创建输出文件夹
    single_dir = "heatmaps_single"
    os.makedirs(single_dir, exist_ok=True)
    print(f"单张热力图(左上)将保存到文件夹: {single_dir}/")

    upper_right_dir = "heatmaps_upper_right"
    os.makedirs(upper_right_dir, exist_ok=True)
    print(f"单张热力图(右上)将保存到文件夹: {upper_right_dir}/")
    ...
    # 指定要处理的三个超参数点（不再遍历所有合法组合）
    # param_points = [(1.04, 1.02), (1.4, 1.10), (1.4, 1.25)]
    param_points = [(1.2,1.08)]

    for idx, (id_val, yd_val) in enumerate(param_points, 1):
        print(f"\nProcessing {idx}/{len(param_points)}: id={id_val:.4f}, yd={yd_val:.4f}")
        ...
        # 生成文件名（保留两位小数，便于区分）
        # filename_single = f"Single_{id_val:.2f}_{yd_val:.2f}.png"
        # save_path_single = os.path.join(single_dir, filename_single)
        # plot_single_heatmap(df_ds, id_val, yd_val, save_path_single)

        filename_upper = f"UpperRight_{id_val:.2f}_{yd_val:.2f}.png"
        save_path_upper = os.path.join(upper_right_dir, filename_upper)
        plot_upper_right_heatmap(df_ds, id_val, yd_val, save_path_upper)
