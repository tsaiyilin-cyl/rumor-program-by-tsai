...
def plot_sign(llm, sign, same_scale=True):
    ...
    if sign == 'positive':
        inf_a = np.array(posinfa)
        inf_b = np.array(posinfb)
        sub_a = np.array(possuba)
        sub_b = np.array(possubb)
    else:
        inf_a = np.array(neginfa)
        inf_b = np.array(neginfb)
        sub_a = np.array(negsuba)
        sub_b = np.array(negsubb)
    ...
    for topic in range(1, 7):
        ...
        for ag in range(6):
            ax.scatter(x_a[ag], y_a[ag], marker=marker, color=agent_colors[ag],
                       s=150, edgecolors='black', linewidth=0.5, alpha=0.5, label=None)
            ax.scatter(x_b[ag], y_b[ag], marker=marker, facecolors='none',
                       edgecolors=baseline_color, s=150, linewidth=1.5, alpha=0.5, label=None)
            all_x_sub1.append(x_a[ag])
            all_y_sub1.append(y_a[ag])
            all_x_sub1.append(x_b[ag])
            all_y_sub1.append(y_b[ag])
    # 中位数线
    median_x = np.median(all_x_sub1)
    median_y = np.median(all_y_sub1)
    ...
    for ag in range(6):
        ax.scatter(mean_inf_a[ag], mean_sub_a[ag], marker='o', color=agent_colors[ag],
                   s=150, edgecolors='black', linewidth=0.5, alpha=0.5, label=agent_labels[ag] if ag == 0 else "")
        ax.scatter(mean_inf_b[ag], mean_sub_b[ag], marker='o', facecolors='none',
                   edgecolors=baseline_color, s=150, linewidth=1.5, alpha=0.5,
                   label='Baseline' if ag == 0 else "")
        all_x_sub2.append(mean_inf_a[ag])
        all_y_sub2.append(mean_sub_a[ag])
        all_x_sub2.append(mean_inf_b[ag])
        all_y_sub2.append(mean_sub_b[ag])
    median_x = np.median(all_x_sub2)
    median_y = np.median(all_y_sub2)
    ax.axvline(median_x, color='red', linestyle='--', linewidth=1, alpha=0.7, label='_nolegend_')
    ax.axhline(median_y, color='blue', linestyle='--', linewidth=1, alpha=0.7, label='_nolegend_')
    ...
    for idx, topic in enumerate(range(1, 7), start=2):
        ...
        for ag in range(6):
            ax.scatter(x_a[ag], y_a[ag], marker=marker, color=agent_colors[ag],
                       s=150, edgecolors='black', linewidth=0.5, alpha=0.5)
            ax.scatter(x_b[ag], y_b[ag], marker=marker, facecolors='none',
                       edgecolors=baseline_color, s=150, linewidth=1.5, alpha=0.5)
            all_x_topic.append(x_a[ag])
            all_y_topic.append(y_a[ag])
            all_x_topic.append(x_b[ag])
            all_y_topic.append(y_b[ag])
        median_x = np.median(all_x_topic)
        median_y = np.median(all_y_topic)
        ax.axvline(median_x, color='red', linestyle='--', linewidth=1, alpha=0.7, label='_nolegend_')
        ax.axhline(median_y, color='blue', linestyle='--', linewidth=1, alpha=0.7, label='_nolegend_')
...
print('All figures generated (same scale with median lines).')