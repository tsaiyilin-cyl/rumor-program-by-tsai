...
def choose_test(data_a, data_b):
    """
    返回 p 值，根据正态性和方差齐性选择 t检验（等方差或不等方差）或 Mann-Whitney U
    """
    # 样本量过小（<3）无法做正态性检验，直接使用非参数检验
    if len(data_a) < 3 or len(data_b) < 3:
        _, p = mannwhitneyu(data_a, data_b, alternative='two-sided')
        print("small len,wallis")
        return p
    ...
    if p_norm_a > 0.05 and p_norm_b > 0.05:
        # 方差齐性检验（Levene）
        _, p_var = levene(data_a, data_b)
        if p_var > 0.05:
            # 方差齐，标准 t检验
            _, p = ttest_ind(data_a, data_b, equal_var=True)
        else:
            # 方差不齐，Welch t检验
            _, p = ttest_ind(data_a, data_b, equal_var=False)
        print("t-test")
        return p
    else:
        # 至少一组非正态，使用非参数检验
                ...
                if agent not in topic_data or not topic_data[agent]['inf'] or not topic_data[agent]['sub']:
                    ...
                    if agent == 6:   # baseline 行，不加星号，黑色
                        ...
                        if not baseline_exists:
                            ...
                            # 影响力检验
                            try:
                                p_inf = choose_test(topic_data[agent]['inf'], topic_data[6]['inf'])
                            except ValueError:
                                p_inf = 1.0
                            ...
                            # 固执度检验
                            try:
                                p_sub = choose_test(topic_data[agent]['sub'], topic_data[6]['sub'])
                            except ValueError:
                                p_sub = 1.0
