        ...
        for item in data:
            ...
            if item['op'] < ini[ts-1][item['agent']]:
                for _ in range(len(ini[ts-1])):
                    if ini[ts-1][_] < ini[ts-1][item['agent']]:
                        num.append(_)
            ...
            if all_val > 0:
                sub[ag] = -all_val
                for idx, src in enumerate(num):
                    contrib = all_val * (weights[idx] / total_weight)
                    inf[src] += contrib
            if ag == 5:
                # 每完成一轮（6个agent），写出数据
                for _ in range(6):
                    sex = _ // 3 + 1
                    race = _ % 3 + 1
                    writer.writerow([sex, race, topic, index, expr, ts, 1, inf[_]])
                    # 写入 subornness (metric=0)
                    writer.writerow([sex, race, topic, index, expr, ts, 0, sub[_]])
                # 清零
                inf = [0 for _ in range(6)]
                sub = [0 for _ in range(6)]

def main():
    csv_file = Path(__file__).parent / "results_addbase_6.csv"  # 新文件名，避免覆盖
    with open(csv_file, 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        # 修改表头：增加 metric 列，合并 influence/subornness 为 value 列
        writer.writerow(["sex", "race", "topic", "llm", "experiment_index", "debate_iter", "metric", "value"])
        model_list = [
            "data_DeepSeek-V3.2",
            "data_GPT-5.1",
