...
class TotLogStream(object):
    ...
    def write_log(cls):
        ...
        if len(cls.model_log) > 0:
            ...
            with open(os.path.join(cls.tar_folder, 'model.txt'), 'a',encoding="utf-8") as f:
                f.write(res_str)
        ...
        for i in range(cls.agent_num):
            ...
            for item in cls.agent_log[i]:
                res_str += json.dumps(item, ensure_ascii=False) + '\n'
            with open(os.path.join(cls.tar_folder, 'agent_{}.txt'.format(i)), 'a',encoding="utf-8") as f:
                f.write(res_str)
        ...
        if cls.event_flag and len(cls.event_log) > 0:
            ...
            for item in cls.event_log:
                res_str += json.dumps(item, ensure_ascii=False) + '\n'
            with open(os.path.join(cls.tar_folder, 'event.txt'), 'a',encoding="utf-8") as f:
                f.write(res_str)
        '''
