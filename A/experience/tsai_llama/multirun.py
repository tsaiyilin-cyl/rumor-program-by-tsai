...
def run_single_experiment(config_file, topic_str, topic_index, support_id, oppose_id, max_iter, log_base_dir, llm, stubborn_flag=False):    #stubborn_flag: True 表示初始具有谣言和非要眼观点的智能体观点不可变，False 表示可变
    ...
    exp_dir = f"topic{topic_index}_base_support{support_id}_oppose{oppose_id}{suffix}"
...
def main():
    ...
    ######
    api_key="sk-MGu4ELGj4lw4OjNgDDCXzNGGb8CYf3AGkIfjujpKKes7iHHo"
    # api_key = os.getenv("DEEPSEEK_API_KEY")
    if not api_key:
        print("no API_KEY")
        sys.exit(1)

    llm = GPTLLM(api_key=api_key,base_url="https://hk.n1n.ai/v1",model="llama-3.1-70b-instruct")
    ...
    for topic_index, topic_str in enumerate(topics):
        for [support_id,oppose_id] in [[1,5],[5,0],[5,1],[5,2],[5,3]]:
            if topic_index != 1: continue
            # for [topic_index,support_id,oppose_id] in skip:
            ...
            # print(time.localtime())
            # run_single_experiment(config_file, topic_str, 0, 1, 5,
            #                       max_iter, log_base_dir, llm, stubborn_flag=False)
            # print(time.localtime())
